...
export default function AdminDashboard() {
  ...
  useEffect(()=>{
    supabase.from("orders_v2").select("total,created_at").then(({data})=>{
      if(!data) return
      ...
      setTimeline(Object.entries(map).map(([date,total])=>({date,total})))
    })

    supabase.from("profiles").select("id").then(({data})=> setCustomers(data?.length||0))

    supabase.from("orders_v2").select("status").then(({data})=>{
      if(!data) return
      const map:any={}
      data.forEach(o=>map[o.status]=(map[o.status]||0)+1)
      setStatusDist(Object.entries(map).map(([name,value])=>({name,value})))
    })

    supabase.from("order_items_v2").select("name,qty").then(({data})=>{
      if(!data) return
      const map:any={}
      data.forEach(i=>map[i.name]=(map[i.name]||0)+i.qty)
      setTopProducts(Object.entries(map).map(([name,qty])=>({name,qty})))
    })
  },[])
  ...
  return (
    <div className="royal-hall space-y-14">
