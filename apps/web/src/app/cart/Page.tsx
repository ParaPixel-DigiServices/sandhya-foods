    ...
    <div className="p-4 max-w-md mx-auto">
      {data.map(i=>(
        <div key={i.id} className="border p-3 mb-3">
          ...
          <button onClick={()=>remove.mutate(i.id)} className="text-red-500">Remove</button>
        </div>
      ))}
      <div className="mt-4 font-bold">Total: ₹{total}</div>
      <a href="/checkout" className="block text-center mt-4 bg-black text-white p-3">Checkout</a>
    </div>
  );
}
